...
def test_subject_blank(subject):
    result = validate_ticket_form(subject, "Valid description", "High", "Open", 2)
    assert result == "Subject cannot be blank."

def test_subject_too_long():
    long_subject = "A" * 101
    result = validate_ticket_form(long_subject, "Valid description", "High", "Open", 2)
...
def test_description_blank(description):
    result = validate_ticket_form("Valid subject", description, "High", "Open", 2)
    assert result == "Description cannot be blank."

def test_description_too_long():
    long_description = "A" * 501
    result = validate_ticket_form("Valid subject", long_description, "High", "Open", 2)
...
def test_estimated_time_invalid(estimated_time):
    ...
    result = validate_ticket_form("Valid subject", "Valid description", "High", "Open", estimated_time)
    assert result == expected_messages[estimated_time]

# Successful Case

def test_valid_ticket_data():
    result = validate_ticket_form("Valid subject", "Valid description", "Medium", "Open", 5)
